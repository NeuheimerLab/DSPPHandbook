...
* The stochastic part represents the error in your model.  This includes error due to all the other possible factors or predictors that have not been included in your hypothesis (called "process error") as well as any error made when you made your observations (called "measurement error").
...
This describes how the data should be assumed to be distributed around your model fit (e.g. how to model the scatter of the data around the line in the figure above).   Note I use the word "assumption" here - we are picking an existing mathematical form ([data distribution](DSPPH_SM_DataDistributions.qmd)) that can approximate the behaviour of your observations.  This is an assumption that you will test in the [model validation section](DSPPH_SM_ModelValidation.qmd) to come.

Here are some [common data distributions](DSPPH_SM_DataDistributions.qmd) that can be used to build your models.
...
One exception: the coefficients for models with a Gamma error distribution are easier to understand when you use a log link instead of the default inverse link.  
...
with the nature of the `Resp` changing with each example.  In each example, the data are given on the response scale on the lefthand side, and the linked scale (mathematical world) on the right hand side.  
...
Also notice that the figures on the response vs. link scale look the same for a normal error distribution assumption.  This is because the GLM math was originally developed for a normal error distribution assumption and then *generalized* for other error distribution assumption.  The link function "identity" that is the canonical link function when you have a normal error distribution assumption tells R that no link is needed.
...
Revist the ["Choose a link function" section](#sec-chooseLink). GLMs can handle situations that appear non-linear in a simple way.  

<!--- include figure of nonlinear vs. linear--->

<!--- also binomial linBinom.png, nonlinBinom.png --->


*Plot your data*: Plotting your response vs. your predictor is another good exercise to get you thinking about what shape assumption will be appropriate. When you make this plot, remember to keep in mind your error distribution assumption choice (choice #1 above).  This will help you understand when you need a linear vs. non-linear model. [We'll look at an example of this when we choose our link function.](#sec-chooseLink).   The GGally package has some good options for quickly plotting your data: 
...
::: {.callout-tip collapse="true" title="What are model coefficients"} 
...
So we can use a GLM as our starting model when we have a shape assumption that is linear, and one of a variety of error distribution assumptions - see a little further along to find out which error distribution assumptions are supported by GLMs^[Note that **generalized** linear models are different than **general** linear model. General linear models include simple linear regression, ANOVA and a few others. Generalized linear models include these but more - they allow us to build models to test many, many different biological hypotheses.  More on this below!] 
...
This output includes^[note that we discuss this more in depth in the [Reporting Section]()]: 
