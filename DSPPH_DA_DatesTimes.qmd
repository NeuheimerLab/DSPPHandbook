...
2) You can extract parts of the date and time column with functions like `months()` for months, `years()` for years, etc.  More generally, you can use the `format()` function.
...
# another way to extract month from your date:
format(myDat$Time, # date object
       "%B") # what you want to extract, %B gives you the month
