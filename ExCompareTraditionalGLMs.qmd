...
* when you need to relate to previous research (e.g. interpreting coefficients, comparing results between your work and existing research that used another method)


## Other types of linear models

Recall from the chapter on [hypothesis testing](DSPPH_SM_HypothesisTesting.qmd), statistical modelling means building a model of your hypothesis so that you can mathematically compare your hypothesis to data and thereby determine the evidence for your hypothesis.  

Remember that models are an approximation of the real world and that "all models are wrong, but some are useful".  It follows then that there could be more than one useful model that can represent your hypothesis.
...
- Classical models rely on P-values to determine the evidence for your hypothesis.  [Refresh how you can use P-values to test your hypothesis here](DSPPH_SM_HypothesisTesting.qmd#sec_PValue).
...
- For some types of observations, there are no equivalent "classical" models - you need a GLM.  This is particularly the case when you have non-normal error distribution assumption that is discrete, such as poisson (count data) or binomial.
...
**This section may seem daunting, but remember that you already know how to address every hypothesis discussed here (with a GLM).  This section is to make you aware of the other options available to you.**
...
* You might see people mentioning Type I vs. Type II vs. Type III ANOVAs.  These are different ANOVA methods used to handle predictor colinearity when selecting which predictors should be in your best-specified model.  You can change the type of ANOVA you are fitting with the `Anova()`^[note the capital letter A here] function in the car package.
