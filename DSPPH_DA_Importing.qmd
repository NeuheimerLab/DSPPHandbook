...
The equals symbol `=` is used for defining arguments within a function ([see more about this here](functions.qmd)).

The double-equals symbol `==` is used for testing to see if two objects are identical.  [You can read more about this here](DSPPH_DA_Exploring.qmd#A conditional view of your data )
...
Use nouns (substantiver) to name data objects, and verbs (verber) to name functions^[more about making your own function soon!]
...
* `dayOne <- ...` (Words are not separated by an underscore)^[This style is called camelcase.  Note that Anna has a habit of using this style... she's working on it...)]
