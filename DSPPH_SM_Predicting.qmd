...
Examples 3 (`Resp3 ~ Num4 + 1`) and 4 (`Resp4 ~ Cat5 + Num6 + Cat5:Num6 + 1`) each contain a numeric predictor, so we will use those here as an example of how to use your models to make a prediction of your response.

### Example 3: Resp3 ~ Num4 + 1 {.unnumbered}

Here's a reminder of Example 3 (`Resp3 ~ Num4 + 1`): 
...
#### What is the predicted response of `Resp3` when `Num4 = 30`?


```{r}

## Step One: Define the predictor values for your response prediction
forPred <- data.frame(Num4 = 30) # the values of my predictor at which I want a response prediction
...
So the value of `Resp3` when `Num4 = 30 units` is `r round(forPred$Fit)` units (95% confidence intervals: `r round(forPred$Lower)` to `r round(forPred$Upper)` units).
...
ggplot() + # start ggplot
  ...
  geom_point(data = forPred, # prediction data frame
             mapping = aes(x = Num4, y = Fit), # coordinates of prediction
  ...
  geom_errorbar(data = forPred, # prediction data frame
             mapping = aes(x = Num4, ymax = Upper, ymin = Lower), # coordinates of prediction
...
#### What are the predicted responses of `Resp3` when `Num4 = 9` and `Num4 = 22`?

```{r}

## Step One: Define the predictor values for your response prediction
forPred <- data.frame(Num4 = c(9,22)) # the values of my predictor at which I want a response prediction
...
```
Note that you now get two predictions, one for `Num4 = 9` and one for `Num4 = 22`:

* The value of `Resp3` when `Num4 = 9 units` is `r round(forPred$Fit[1])` units (95% confidence intervals: `r round(forPred$Lower[1])` to `r round(forPred$Upper[1])` units).

* The value of `Resp3` when `Num4 = 22 units` is `r round(forPred$Fit[2])` units (95% confidence intervals: `r round(forPred$Lower[2])` to `r round(forPred$Upper[2])` units).
...
### Example 4: Resp4 ~ Cat5 + Num6 + Cat5:Num6 + 1 {.unnumbered}


Here's a reminder of Example 4 (`Resp4 ~ Cat5 + Num6 + Cat5:Num6 + 1`): 
...
#### What is the predicted response of `Resp4` when `Num6 = 800` for `Cat5 = Urban`?


```{r}

## Step One: Define the predictor values for your response prediction
forPred <- data.frame(Num6 = 800,
...
So the value of `Resp4` when `Num6 = 800` units for `Cat5 = Urban` is `r round(forPred$Fit,1)` units (95% confidence intervals: `r round(forPred$Lower,1)` to `r round(forPred$Upper,1)` units).
...
ggplot() + # start ggplot
  ...
  geom_line(data = forVis, # add the modelled fit to your plot
    ...
    ### NEW!  Adding your response prediction to the plot:
  geom_point(data = forPred, # prediction data frame
             mapping = aes(x = Num6, y = Fit, col = Cat5), # coordinates of prediction
             #col = "black", # change point colour
             size = 3) + # change point size
  
  ### NEW!  Adding errorbars for your response prediction to the plot:
  geom_errorbar(data = forPred, # prediction data frame
             mapping = aes(x = Num6, ymax = Upper, ymin = Lower,  col = Cat5), # coordinates of prediction
...
#### What is the predicted response of `Resp4` when `Num6 = 800` for both `Cat5 = Urban` and `Cat5 = Wild`?
...
forNum6 <- 800 # value of Num6 for prediction

forPred <- expand.grid(Cat5 = forCat5, Num6 = forNum6) # expand.grid() function makes sure you have all combinations of predictors.
...
* the value of `Resp4` when `Num6 = 800` units is  both `Cat5 = Urban` is `r round(forPred$Fit[1],1)` units (95% confidence intervals: `r round(forPred$Lower[1],1)` to `r round(forPred$Upper[1],1)` units).

* the value of `Resp4` when `Num6 = 800` units is  both `Cat5 = Wild` is `r round(forPred$Fit[2],1)` units (95% confidence intervals: `r round(forPred$Lower[2],1)` to `r round(forPred$Upper[2],1)` units).
...
```
## A special case: Inverse predicting for models with a binomial error distribution assumption
...
One method to do this would be to predict your response at values of your predictor in high enough resolution so that you can pick out the appropriate predictor value.  Here is an example where you want to know the value of `Num4` that will give a 75% probability of `Resp3.b` being 1, given your bestmod3.b of `Resp3.b ~ Num4 + 1`:

```{r}

forNum4<-seq(from = min(myDat3.b$Num4), to = max(myDat3.b$Num4), by = 0.01)# a sequence of numbers in your numeric predictor range, notice that you are using a very high resolution series of your predictor here.

forPred <- expand.grid(Num4 = forNum4) # expand.grid() function makes sure you have all combinations of predictors.
...
myPredDF <- data.frame(Num4 = forNum4,
                       Fit = forPred$Fit,
                       Lower = forPred$Lower,
                       Upper = forPred$Upper)
...
```
The estimate of `Num4` that leads to a 75% probability of `Resp3.b` being 1 is `r round(myInvPred$Fit, 1)` units (95% confidence intervals: `r round(myInvPred$Lower, 2)` to `r round(myInvPred$Upper, 2)`.
...
Then we can solve $x$ for a particular value of $p$.  For example, what value of `Num4` will give a 75% probability of `Resp3.b` being 1, given your bestmod3.b of `Resp3.b ~ Num4 + 1`:
...
funInvPred <- function (p, model) {
  ...
  (modLink(p) - coef(model)[["(Intercept)"]]) / coef(model)[["Num4"]] # calculate x from p
}

# Apply the equation to estimate Num4 when there is a 75% probability of Resp3.b being 1:
...
Up next we will practice applying the framework to new examples, and learn how to [communicate](DSPPH_SM_Communicating.qmd) the whole process in reports and papers.
