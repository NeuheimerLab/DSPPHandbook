...
You can fit and rank your models quickly using a function called `dredge()` in the MuMIn package^[note the spelling and capitalization of this package name!].  The `dredge()` function fits and ranks models representing all possible predictor combinations based on your starting model - i.e. your default candidate model set^[other methods include forwards or backwards model selection.  In these, you add (or subtract) one predictor term from your starting model to determine the effect on your model's ability to explain the variability in your response.  These methods can be biased by collinearity among your predictors in your model, so we will proceed with looking at all possible predictor combinations as described here].  The output is a table ranking the models in your candidate model set.  
