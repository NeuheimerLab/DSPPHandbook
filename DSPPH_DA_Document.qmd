...
*EXTRA*: It's fine to try out syntax at the command prompt (i.e. in interactive mode) before copying and pasting it into your script.  Just be sure not to copy over the command prompt ("`>`") itself as then you'll get an error when you try to run commands from your script.  
...
*EXTRA*: You can also send the entire script to be executed all at once with `source("ScriptName.R")` at the command prompt in R, or `R CMD ScriptName.R` at the system shell command. 
